...
from utils.colors import COLOR_LABELS
...
def main():
    ...
    color_payload = None if not color_str or color_str.lower() == "none" else color_str.lower()
    valid_colors = set(COLOR_LABELS)
    if color_payload and color_payload not in valid_colors:
        print(f"[set_track_color] invalid color '{color_payload}'. Allowed: {', '.join(sorted(valid_colors))}", file=sys.stderr)
        sys.exit(1)
